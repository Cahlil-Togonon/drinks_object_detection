...
def get_args():
    ...
    parser.add_argument("--checkpoint", type=str, default="./kws_models/kws-transformer-best-acc.ckpt")
...
if __name__ == "__main__":
    ...
    scripted_module = model.load_from_checkpoint(checkpoint)
    ...
    while True:
        ...
        if args.rpi:
            ...
            mel = ToTensor()(librosa.power_to_db(transform(waveform).squeeze().numpy(), ref=np.max))
        mel = mel.unsqueeze(0)
        mel = rearrange(mel, 'b c (p1 h) (p2 w) -> b (p1 p2) (c h w)', p1=4, p2=4) # change mel to 4x4 patches
